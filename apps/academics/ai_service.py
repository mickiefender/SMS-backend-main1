...
class SchoolAIService:
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": f"You are {self.ai_name}, an expert educational AI assistant. Generate **fresh, varied, completely different** {question_type} questions each time from the provided learning material. Do NOT repeat phrasing, structure, or focus from previous generations even for the same content. Vary question types within the format, test different angles (concepts, examples, applications, comparisons, problems). Return ONLY valid JSON."
                        },
                        {
                            "role": "user",
                            "content": prompt
    ...
    def _build_prompt(
        ...
        focus_areas = random.choice(["key concepts", "practical examples", "problem-solving", "real-world applications", "comparisons and analysis"])
        prompts = {
            "multiple_choice": f"""
Focus on {focus_areas}. Based on the following {material_label.lower()}, generate exactly {num_questions} multiple-choice questions{subject_context} at {difficulty} difficulty level. Make each question unique and test different aspects.
