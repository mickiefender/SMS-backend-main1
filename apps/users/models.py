...
class TeacherProfile(models.Model):
    ...
    def save(self, *args, **kwargs):
        print(f"[DEBUG] TeacherProfile.save: user_id={getattr(self.user, 'id', None)}, school_id={getattr(self.user, 'school_id', getattr(self.user.school, 'id', None))}")
        
        if not self.employee_id:
            try:
                if not self.user or not self.user.id:
                    raise ValueError("No user or user.id")
                ...
                if self.user.school:
                    with transaction.atomic():
                        ...
                        school_initials = ''.join([word[0].upper() for word in words if word])[:3] or "SCH"
                        year = datetime.now().year
                        
                        last_teacher = TeacherProfile.objects.filter(
                            user__school=self.user.school,
                            employee_id__startswith=f"{school_initials}{year}"
                        ).order_by('employee_id').last()
                        
                        if last_teacher and last_teacher.employee_id:
                            try:
                                last_count = int(last_teacher.employee_id[-5:])
                                new_count = last_count + 1
                            except (ValueError, IndexError):
                                new_count = TeacherProfile.objects.filter(
                                    user__school=self.user.school, 
                        ...
                        self.employee_id = f"{school_initials}{year}{new_count:05d}"
                        print(f"[DEBUG] Generated school-based employee_id: {self.employee_id}")
                else:
                    raise ValueError("No school associated with user")
            except Exception as gen_err:
                print(f"[DEBUG] Employee ID generation failed: {str(gen_err)}, using fallback")
                self.employee_id = f"EMP{self.user.id:05d}"
                print(f"[DEBUG] Fallback employee_id: {self.employee_id}")
        
        print(f"[DEBUG] Final employee_id before save: {self.employee_id}")
        super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.user.get_full_name()} - Teacher ({self.employee_id})"
